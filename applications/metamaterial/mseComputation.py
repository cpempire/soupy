...
def mseRun(type):
    ...
    for i in range(N_mse):
        ...
        u, Q_mean[i], lin_diff_mean[i], quad_diff_mean[i], Q_var[i], lin_diff_var[i], quad_diff_var[i] \
            = cost.QuadraticSampling(sample, savedata=True, i=i, type=type)
        u_mean.axpy(1.0, u)
        u[:] = u[:]*u[:]
        u_var.axpy(1.0, u)
    ...
    filename_ur = "data/"+"ur_"+type+'_mean.xdmf'
    filename_ui = "data/"+"ui_"+type+'_mean.xdmf'
    if dlversion() <= (1, 6, 0):
    ...
    filename_ur = "data/"+"ur_"+type+'_var.xdmf'
    filename_ui = "data/"+"ui_"+type+'_var.xdmf'
    if dlversion() <= (1, 6, 0):
    ...
    if mpi_rank == 0:
        np.savez("data/mseResults"+type+".npz", Q_mean=Q_mean,lin_diff_mean=lin_diff_mean,quad_diff_mean=quad_diff_mean,
...
cost = CostFunctionalQuadratic(parameter, Vh, pde, qoi, prior, penalization, tol=1e-8)
