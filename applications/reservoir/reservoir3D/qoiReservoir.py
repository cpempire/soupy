...
class QoIConstraint(QoI):
    ...
    def __init__(self, mesh, Vh):
        ...
        self.f_c = dl.interpolate(dl.Constant(4.), Vh[STATE])
        self.chi = dl.Expression("128.*(x[0]<=0.75)*(x[0]>=0.25)*(x[1]<=0.75)*(x[1]>=0.25)", degree=1)
    ...
    def form(self, u, m, z):
        ...
        # print("f = ", dl.assemble(self.chi * u * u * dl.dx), "fc = ", dl.assemble(self.f_c * dl.dx))
