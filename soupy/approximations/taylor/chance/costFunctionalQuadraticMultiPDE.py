...
class CostFunctionalQuadraticMultiPDE(CostFunctional):
    """
    ...
    def __init__(self, parameter, Vh, pde, qoi, prior, penalization, tol=1e-9):
        ...
        self.dmr, self.const, self.dmmr, self.Q_0 = [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde

        self.mhelp, self.Hmhat1 = [None]*self.N_pde, [None]*self.N_pde

        self.rhs_fwd, self.rhs_adj, self.rhs_adj2, self.rhs_adj3, self.rhs_adj4 = [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde

        self.xhat, self.yhat, self.xhatstar, self.yhatstar, self.mhat, self.mhatstar = \
            [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde

        self.d, self.U, self.H = [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde
        ...
        for n_pde in range(self.N_pde):
            ...
            self.const[n_pde] = np.zeros(self.N_mc)
            self.dmmr[n_pde] = np.zeros(self.N_mc)
            self.Q_0[n_pde] = 0.

            self.mhelp[n_pde] = pde[n_pde].generate_parameter()
            self.Hmhat1[n_pde] = pde[n_pde].generate_parameter()

            self.rhs_fwd[n_pde] = pde[n_pde].generate_state()
            self.rhs_adj[n_pde] = pde[n_pde].generate_state()
            self.rhs_adj2[n_pde] = pde[n_pde].generate_state()
            self.rhs_adj3[n_pde] = pde[n_pde].generate_state()


            self.xhat[n_pde] = dict()
            self.yhat[n_pde] = dict()
            self.xhatstar[n_pde] = dict()
            self.yhatstar[n_pde] = dict()
            self.mhat[n_pde] = dict()
            self.mhatstar[n_pde] = dict()

            for i in range(self.N_tr):
                self.xhat[n_pde][i] = pde[n_pde].generate_state()
                self.yhat[n_pde][i] = pde[n_pde].generate_state()
                self.mhat[n_pde][i] = pde[n_pde].generate_parameter()
                self.xhatstar[n_pde][i] = pde[n_pde].generate_state()
                self.yhatstar[n_pde][i] = pde[n_pde].generate_state()
                self.mhatstar[n_pde][i] = pde[n_pde].generate_parameter()
    ...
    def objectiveLinear(self):
        ...
        for n_pde in range(self.N_pde):
            ...
            Q_0 = self.qoi[n_pde].eval(self.x_all[n_pde]) # Q(x(m_0))
            self.Q_0[n_pde] = Q_0

            # solve the adjoint problem at the mean
            rhs = self.pde[n_pde].generate_state()
            self.qoi[n_pde].adj_rhs(self.x_all[n_pde], rhs)
            ...
            self.qoi[n_pde].setLinearizationPoint(self.x_all[n_pde])
            ...
            if self.correction:
                for i in range(self.N_mc):
                    ...
                    self.x_mc[n_pde][i] = x
                    x_all[STATE] = x
                    Q_i = self.qoi[n_pde].eval(x_all) #Q(x(m_i))
    ...
    def objective(self):
        ...
        cost = 0.
        for n_pde in range(self.N_pde):
            self.H[n_pde] = ReducedHessianSVD(self.pde[n_pde], self.qoi[n_pde], self.tol)
            self.d[n_pde], self.U[n_pde] = doublePassG(self.H[n_pde], self.prior.R, self.prior.Rsolver, self.Omega, self.N_tr, s = 1)
                        # check_Bortho = True, check_Aortho=True, check_residual = True)

            for i in range(self.N_tr):
                self.mhat[n_pde][i].zero()
                self.mhat[n_pde][i].axpy(1., self.U[n_pde][i])
            ...
            self.quad_mean[n_pde] = self.Q_0[n_pde] + 0.5*np.sum(self.d[n_pde])
            self.quad_var[n_pde] = self.lin_var[n_pde] + 0.25*(np.sum(self.d[n_pde]))**2 \
                                   + 0.5*np.sum(np.square(self.d[n_pde])) + self.Q_0[n_pde]*np.sum(self.d[n_pde])

            for i in range(self.N_tr):
                dmmr, xhat, yhat = self.HessianInner(self.mhat[n_pde][i], self.mhat[n_pde][i], n_pde)
                self.xhat[n_pde][i] = xhat
                self.yhat[n_pde][i] = yhat

            if self.correction:
                for i in range(self.N_mc):
                    dmmr, xhat, yhat = self.HessianInner(self.m_mc[n_pde][i], self.m_mc[n_pde][i], n_pde)
                    self.dmmr[n_pde][i] = dmmr
                    self.xhat[n_pde][i+self.N_tr] = xhat
                    self.yhat[n_pde][i+self.N_tr] = yhat

                    self.quad_diff_mean[n_pde][i] = self.Q_mc[n_pde][i] - (self.Q_0[n_pde]+self.dmr[n_pde][i]+1./2*self.dmmr[n_pde][i])
                    self.quad_fval_mean[n_pde][i] = self.Q_mc[n_pde][i]

                    self.quad_diff_var[n_pde][i] = (self.Q_mc[n_pde][i])**2 - (self.Q_0[n_pde]+self.dmr[n_pde][i]+1./2*self.dmmr[n_pde][i])**2
                    self.quad_fval_var[n_pde][i] = (self.Q_mc[n_pde][i])**2 # (Q_i)^2

                    self.const[n_pde][i] = -0.5/self.N_mc \
                                    -self.beta/self.N_mc*(self.Q_0[n_pde]+self.dmr[n_pde][i]+1./2*self.dmmr[n_pde][i]) \
                                    +self.beta/self.N_mc*(self.quad_mean[n_pde] + np.mean(self.quad_diff_mean[n_pde]))
                    # print "######################################################################################"
            ...
            for i in range(self.N_tr):
                self.mhatstar[n_pde][i].zero()
                if self.correction:
                    self.mhatstar[n_pde][i].axpy(0.5+self.beta*self.d[n_pde][i]-self.beta*np.mean(self.quad_diff_mean[n_pde]), self.U[n_pde][i])
                else:
                    self.mhatstar[n_pde][i].axpy(0.5+self.beta*self.d[n_pde][i], self.U[n_pde][i])

            cost += self.quad_mean[n_pde] + np.mean(self.quad_diff_mean[n_pde]) \
                        + self.beta*(self.quad_var[n_pde] + np.mean(self.quad_diff_var[n_pde])) \
                        - self.beta*(self.quad_mean[n_pde] + np.mean(self.quad_diff_mean[n_pde]))**2

            if self.mpi_rank == 0:
                print("#######################quadratic approximation#########################")
                header = ["mean", "mean_diff", "var", "var_diff"]
                print('{:<20} {:<20} {:<20} {:<20}'.format(*header))
                data = [self.quad_mean[n_pde], np.mean(self.quad_diff_mean[n_pde]),
                        self.quad_var[n_pde] - self.quad_mean[n_pde]**2, np.mean(self.quad_diff_var[n_pde])]
                print('{:<20} {:<20} {:<20} {:<20}'.format(*data))
                print("####################################################################")
                ...
                if self.correction:
                    ...
                    print('{:<20} {:<20} {:<20} {:<20}'.format(*header))
                    data = [np.var(self.quad_fval_mean[n_pde])/self.N_mc, np.var(self.quad_diff_mean[n_pde])/self.N_mc,
                            np.var(self.quad_fval_var[n_pde])/self.N_mc, np.var(self.quad_diff_var[n_pde])/self.N_mc]
                    print('{:<20} {:<20} {:<20} {:<20}'.format(*data))
                    print ("######################################################################################")

        return cost

    def QuadraticSampling(self, sample, savedata=False, i=0, type='random', n_pde=0): # to add N_pde

        m = self.pde[n_pde].generate_parameter()
        m.axpy(1., self.m)
        m.axpy(1., sample)
        x_all = [self.x[n_pde], m, self.y[n_pde], self.z]
        x = self.pde[n_pde].generate_state()
        self.pde[n_pde].solveFwd(x, x_all, self.tol)
        ...
        dmmr, xhat, yhat = self.HessianInner(sample, sample, n_pde)
        x_all[STATE] = x
        Q_mean = self.qoi[n_pde].eval(x_all) # Q(x(m_i))
        lin_diff_mean = Q_mean - self.Q_0[n_pde] - dmr # the gradient term
        quad_diff_mean = lin_diff_mean - dmmr/2

        Q_var = (Q_mean)**2
        lin_diff_var = Q_var - (self.Q_0[n_pde]+dmr)**2
        quad_diff_var = Q_var - (self.Q_0[n_pde]+dmr+dmmr/2)**2

        if savedata:
            x_fun = vector2Function(x, self.Vh[STATE])
            filename = "data/u_" + type + "_PDE_" + str(n_pde) + "_sample_" + str(i) +"_load.h5"
    ...
    def ForwardSolution(self, sample): # to add N_pde
    ...
    # Hessian Action H*m
    def HessianAction(self, mhat1): # to add N_pde
    ...
    def HessianInner(self, mhat1, mhat2, n_pde):
        # check Hessian by finite difference
        xhat = self.pde[n_pde].generate_state()
        yhat = self.pde[n_pde].generate_state()
        self.pde[n_pde].apply_ij(ADJOINT,PARAMETER, mhat1, self.rhs_fwd[n_pde])
        self.pde[n_pde].solveIncremental(xhat, -self.rhs_fwd[n_pde], False, self.tol) # False for forward,
        self.pde[n_pde].apply_ij(STATE, STATE, xhat, self.rhs_adj[n_pde])
        self.pde[n_pde].apply_ij(STATE, PARAMETER, mhat1, self.rhs_adj2[n_pde])
        self.rhs_adj[n_pde].axpy(1., self.rhs_adj2[n_pde])
        self.qoi[n_pde].apply_ij(STATE,STATE,xhat,self.rhs_adj3[n_pde])
        self.rhs_adj[n_pde].axpy(1., self.rhs_adj3[n_pde])
        self.qoi[n_pde].apply_ij(STATE, PARAMETER, mhat1, self.rhs_adj4[n_pde])
        self.rhs_adj[n_pde].axpy(1., self.rhs_adj4[n_pde])
        self.pde[n_pde].solveIncremental(yhat, -self.rhs_adj[n_pde], True, self.tol) # True for adjoint,
        self.pde[n_pde].apply_ij(PARAMETER,PARAMETER, mhat1, self.Hmhat1[n_pde])
        self.pde[n_pde].apply_ij(PARAMETER,ADJOINT,yhat, self.mhelp[n_pde])
        self.Hmhat1[n_pde].axpy(1., self.mhelp[n_pde])
        self.pde[n_pde].apply_ij(PARAMETER,STATE, xhat, self.mhelp[n_pde])
        self.Hmhat1[n_pde].axpy(1., self.mhelp[n_pde])
        self.qoi[n_pde].apply_ij(PARAMETER, ADJOINT, yhat, self.mhelp[n_pde])
        self.Hmhat1[n_pde].axpy(1., self.mhelp[n_pde])
        self.qoi[n_pde].apply_ij(PARAMETER, STATE, xhat, self.mhelp[n_pde])
        self.Hmhat1[n_pde].axpy(1., self.mhelp[n_pde])

        return mhat2.inner(self.Hmhat1[n_pde]), xhat, yhat
    ...
    def solveAdjIncrementalAdj(self):
        ...
        for n_pde in range(self.N_pde):
            for i in range(self.N_tr):
                xhatstar = self.pde[n_pde].generate_state()
                if self.correction:
                    xhatstar.axpy((0.5+self.beta*self.d[n_pde][i]-self.beta*np.mean(self.quad_diff_mean[n_pde])), self.xhat[n_pde][i])
                    self.xhatstar[n_pde][i] = xhatstar
                else:
                    xhatstar.axpy((0.5+self.beta*self.d[n_pde][i]), self.xhat[n_pde][i])
                    self.xhatstar[n_pde][i] = xhatstar

            if self.correction:
                for i in range(self.N_mc):
                    xhatstar = self.pde[n_pde].generate_state()
                    dmyr = self.pde[n_pde].forSolveAdjIncrementalAdj(self.x_all[n_pde], self.m_mc[n_pde][i])
                    xhatstarrhs = self.pde[n_pde].generate_state()

                    xhatstarrhs.axpy(self.const[n_pde][i], dmyr)
                    self.pde[n_pde].solver_fwd_inc.solve(xhatstar, -xhatstarrhs)
                    self.xhatstar[n_pde][i+self.N_tr] = xhatstar

    def solveAdjIncrementalFwd(self):

        for n_pde in range(self.N_pde):
            for i in range(self.N_tr):
                yhatstar = self.pde[n_pde].generate_state()
                if self.correction:
                    yhatstar.axpy((0.5+self.beta*self.d[n_pde][i]-self.beta*np.mean(self.quad_diff_mean[n_pde])), self.yhat[n_pde][i])
                    self.yhatstar[n_pde][i] = yhatstar
                else:
                    yhatstar.axpy((0.5+self.beta*self.d[n_pde][i]), self.yhat[n_pde][i])
                    self.yhatstar[n_pde][i] = yhatstar
            ...
            if self.correction:
                ...
                for i in range(self.N_mc):
                    yhatstar = self.pde[n_pde].generate_state()
                    dmxr, dxxr, dxxq = \
                        self.pde[n_pde].forSolveAdjIncrementalFwd(self.x_all[n_pde], self.m_mc[n_pde][i], self.xhatstar[n_pde][i+self.N_tr], self.qoi[n_pde])
                    yhatstarrhs = self.pde[n_pde].generate_state()

                    yhatstarrhs.axpy(self.const[n_pde][i], dmxr)
                    yhatstarrhs.axpy(1., dxxr)
                    yhatstarrhs.axpy(1., dxxq)
                    self.pde[n_pde].solver_adj_inc.solve(yhatstar, -yhatstarrhs)
                    self.yhatstar[n_pde][i+self.N_tr] = yhatstar
    ...
    def solveAdjAdj(self):
        ...
        for n_pde in range(self.N_pde):
            ...
            [bc.apply(dmyr) for bc in self.pde[n_pde].bc0]
            # self.pde.bc0.apply(dmyr)
            xstarrhs.axpy(2*self.beta, dmyr)

            for i in range(self.N_tr):
                dmyr, dxxyr, dxmyr, dmmyr, dmxyr = \
                    self.pde[n_pde].forSolveAdjAdj(self.x_all[n_pde],self.xhat[n_pde][i],
                                                   self.xhatstar[n_pde][i],self.mhat[n_pde][i],self.mhatstar[n_pde][i])
            ...
            if self.correction:
                for i in range(self.N_mc):
                    dmyr, dxxyr, dxmyr, dmmyr, dmxyr = \
                        self.pde[n_pde].forSolveAdjAdj(self.x_all[n_pde],self.xhat[n_pde][i+self.N_tr],
                                                       self.xhatstar[n_pde][i+self.N_tr],self.m_mc[n_pde][i],self.m_mc[n_pde][i])

                    xstarrhs.axpy(1., dxxyr)
                    xstarrhs.axpy(1., dxmyr)

                    xstarrhs.axpy(2.*self.const[n_pde][i], dmyr)
                    xstarrhs.axpy(self.const[n_pde][i], dmmyr)
                    xstarrhs.axpy(self.const[n_pde][i], dmxyr)

            self.pde[n_pde].solver_fwd_inc.solve(self.xstar[n_pde], -xstarrhs)

    def solveAdjFwd(self):

        for n_pde in range(self.N_pde):
            ystarrhs = self.pde[n_pde].generate_state()
            ystarrhsqoi = self.pde[n_pde].generate_state()
            self.qoi[n_pde].grad_state(self.x_all[n_pde], ystarrhsqoi)
            ystarrhs.axpy(1., ystarrhsqoi)
            ystarrhspde = self.pde[n_pde].generate_state()
            self.qoi[n_pde].apply_ij(STATE, STATE, self.xstar[n_pde], ystarrhspde)
            ystarrhs.axpy(1., ystarrhspde)
            self.pde[n_pde].apply_ij(STATE, STATE, self.xstar[n_pde], ystarrhspde)
            ystarrhs.axpy(1., ystarrhspde)
            [bc.apply(ystarrhs) for bc in self.pde[n_pde].bc0]
            ...
            [bc.apply(dmxr) for bc in self.pde[n_pde].bc0]
            # self.pde.bc0.apply(dmxr)
            ystarrhs.axpy(2.*self.beta, dmxr)

            for i in range(self.N_tr):
                mhat = self.mhat[n_pde][i]
                mhatstar = self.mhatstar[n_pde][i]
                xhat = self.xhat[n_pde][i]
                xhatstar = self.xhatstar[n_pde][i]
                yhat = self.yhat[n_pde][i]
                yhatstar = self.yhatstar[n_pde][i]

                dmxr, dyxxr, dymxr, dxyxr, dxxxr, dxmxr, dxxxq, dmmxr, dmyxr, dmxxr,  dxmxq, dmmxq, dmxxq = \
                    self.pde[n_pde].forSolveAdjFwd(self.x_all[n_pde], xhat, xhatstar, mhat, mhatstar, yhat, yhatstar, self.qoi[n_pde])
            ...
            if self.correction:
                ystarrhs.axpy(-2.*self.beta*np.mean(self.quad_diff_mean[n_pde]),
                              ystarrhsqoi)
                for i in range(self.N_mc):
                    mhat = self.m_mc[n_pde][i]
                    mhatstar = self.m_mc[n_pde][i]
                    xhat = self.xhat[n_pde][i+self.N_tr]
                    xhatstar = self.xhatstar[n_pde][i+self.N_tr]
                    yhat = self.yhat[n_pde][i+self.N_tr]
                    yhatstar = self.yhatstar[n_pde][i+self.N_tr]
                    ...
                    ystarrhs.axpy(2.*self.const[n_pde][i], dmxr)

                    ystarrhs.axpy(2*self.const[n_pde][i], ystarrhsqoi)

                    ystarrhs.axpy(self.const[n_pde][i], dmmxr)
                    ystarrhs.axpy(self.const[n_pde][i], dmxxr)
                    ystarrhs.axpy(self.const[n_pde][i], dmyxr)

                    ystarrhs.axpy(1., dxmxq)
                    ystarrhs.axpy(self.const[i], dmmxq)
                    ystarrhs.axpy(self.const[i], dmxxq)

            self.pde[n_pde].solver_adj_inc.solve(self.ystar[n_pde], -ystarrhs)
    ...
    def solve_ymc(self):
        ...
        for n_pde in range(self.N_pde):
            for i in range(self.N_mc):
                ...
                m_i.axpy(1., m)
                m_i.axpy(1., self.m)
                x_all = [self.x_mc[n_pde][i], m_i, self.y[n_pde], self.z]
                rhs = self.pde[n_pde].generate_state()
                self.qoi[n_pde].adj_rhs(x_all, rhs)
                [bc.apply(rhs) for bc in self.pde[n_pde].bc0]

                rhs[:] *= 1./self.N_mc*(1.
                                        + 2.*self.beta*(self.quad_fval_mean[n_pde][i])
                                        - 2.*self.beta*(self.quad_mean[n_pde]+np.mean(self.quad_diff_mean[n_pde])))
    ...
    def costGradient(self, z):
        ...
        for n_pde in range(self.N_pde):
            ...
            q_form = self.qoi[n_pde].form(self.x_all[n_pde])
            ...
            if self.correction:
                for i in range(self.N_mc):
                    ...
                    x_mc = self.x_mc[n_pde][i]
                    x_fun = vector2Function(x_mc, self.Vh[STATE])
                    y_mc = self.y_mc[n_pde][i]
            ...
            for i in range(self.N_tr):
                ...
                dyzr, dxzr, dyxzr, dymzr, dxyzr, dxxzr, dxmzr, dmmzr, dmyzr, dmxzr, dmxzq, dmmzq, dxxzq, dxmzq = \
                    self.pde[n_pde].gradientControl(self.x_all[n_pde],self.xstar[n_pde],self.ystar[n_pde],xhat,xhatstar,mhat,mhatstar,yhat,yhatstar, self.qoi[n_pde])
            ...
            if self.correction:
                for i in range(self.N_mc):
                    ...
                    dzq.axpy(2*self.const[n_pde][i], dmzr)
                    dzq.axpy(self.const[n_pde][i], dmmzr)
                    dzq.axpy(self.const[n_pde][i], dmxzr)
                    dzq.axpy(self.const[n_pde][i], dmyzr)

                    dzq.axpy(1.,dxxzq)
                    dzq.axpy(1.,dxmzq)
                    dzq.axpy(self.const[n_pde][i], dmmzq)
                    dzq.axpy(self.const[n_pde][i], dmxzq)

        dz = self.generate_optimization()
        dz.axpy(1., dzq)

        dzp = self.generate_optimization()
