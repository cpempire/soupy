...
class ChanceConstraintQuadratic(CostFunctional):
    ...
    def objective(self):
        ...
        self.d, self.U = doublePassG(self.H, self.prior.R, self.prior.Rsolver, self.Omega, self.N_tr, s=2)
    ...
    def constraint(self):
        ...
        self.d_constraint, self.U_constraint = doublePassG(self.H_constraint, self.prior.R, self.prior.Rsolver, self.Omega, self.N_tr, s=2)
                    # check_Bortho = True, check_Aortho=True, check_residual = True)
        ...
        # to compute the constant for adjoint of incremental variables
        for i in range(self.N_tr):
            c_f = 0.
            for j in range(self.N_cc):
                ...
                c_f += 1. / (2 * self.N_cc) * ds_gamma * dl_beta * (self.CinvU_constraint[i].inner(self.m_cc[j])) ** 2
            self.c_f[i] = c_f
    ...
    def costGradient(self, z):
        ...
        for i in range(self.N_tr):
            ...
            dyzr, dxzr, dyxzr, dymzr, dxyzr, dxxzr, dxmzr, dmmzr, dmyzr, dmxzr, dmxzq, dmmzq, dxxzq, dxmzq = \
                self.pde.gradientControl(self.x_all,self.xstar,self.ystar,xhat,xhatstar,mhat,mhatstar,yhat,yhatstar, self.qoi)
