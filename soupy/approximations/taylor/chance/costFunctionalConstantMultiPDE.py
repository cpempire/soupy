...
class CostFunctionalConstantMultiPDE(CostFunctional):
    """
    ...
    def __init__(self, parameter, Vh, pde, qoi, prior, penalization, tol=1e-9):
        ...
        self.x, self.y, self.x_all = [], [], []
        for i in range(self.N_pde):
            self.x.append(pde[i].generate_state())
            self.y.append(pde[i].generate_state())
            self.x_all.append([self.x[i], self.m, self.y[i], self.z])
    ...
    def costGradient(self, z):
        ...
        for n_pde in range(self.N_pde):
            ...
            dzqi = self.generate_optimization()
            self.pde[n_pde].evalGradientControl(self.x_all[n_pde], dzqi)
            dzq.axpy(1., dzqi)
