...
class ControlPDEProblem(PDEProblem):
    ...
    def solveFwd(self, state, x, tol=None):
        ...
        if self.is_fwd_linear:
            ...
            F = self.residual(u, m, p, z)
            J = dl.derivative(F, u, u_trial)
            nonlinearproblem = dl.NonlinearVariationalProblem(F, u, self.bc, J)
            solver = dl.NonlinearVariationalSolver(nonlinearproblem)
    ...
    def solveAdj(self, adj, x, adj_rhs, tol=None):
    ...
    def setLinearizationPoint(self, x, gauss_newton_approx = False):
        ...
        if self.solver_fwd_inc is None:
            self.solver_fwd_inc = self._createSolver()
            self.solver_adj_inc = self._createSolver()
