...
class ChanceConstraintConstant(CostFunctional):
    ...
    def constraint(self):
        ...
        self.f_0 = self.qoi_constraint.eval(self.x_all)

        l_beta = 1./(1+np.exp(-2*self.c_beta*self.f_0))
        ...
        if self.mpi_rank == 0:
            print("P(f_taylor>= 0) = ", np.double(self.f_0>=0), " E[\ell_beta(f)] = ", self.chance + self.c_alpha)
    ...
    def costGradient(self, z):
        ...
        if self.c_gamma > 0:
            ...
            dl_beta = 2.*self.c_beta*np.exp(-2.*self.c_beta*self.f_0) / (1 + np.exp(-2.*self.c_beta*self.f_0))**2
