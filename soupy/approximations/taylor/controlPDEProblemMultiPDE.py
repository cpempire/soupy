...
class ControlPDEProblemMultiPDE(PDEProblem):
    ...
    def __init__(self, Vh, residual, bc, bc0, is_fwd_linear=False, data=None):
        self.Vh = Vh
    ...
    def solveFwd(self, state, x, tol):
        ...
        if self.is_fwd_linear:
            ...
            res_form = self.residual(u, m, p, z, data=self.data)
    ...
    def setLinearizationPoint(self, x, gauss_newton_approx = False):
        ...
        f_form = self.residual(*x_fun, data=self.data)
...
##############################################################
    ...
    def forSolveAdjIncrementalAdj(self, x, mhat):
        ...
        f_form = self.residual(u, m, p, z, data=self.data)
    ...
    def forSolveAdjFwd(self, x, uhat, uhatstar, mhat, mhatstar, phat, phatstar, qoi):
        ...
        return dmxr_ass, dyxxr, dymxr, dxyxr, dxxxr, dxmxr, dxxxq, dmmxr, dmyxr, dmxxr

    def gradientControl(self, x, ustar, pstar, uhat, uhatstar, mhat, mhatstar, phat, phatstar):
        u = vector2Function(x[STATE], self.Vh[STATE])
        m = vector2Function(x[PARAMETER], self.Vh[PARAMETER])
        p = vector2Function(x[ADJOINT], self.Vh[ADJOINT])
        z = vector2Function(x[OPTIMIZATION], self.Vh[OPTIMIZATION])
        ...
        return dyzr, dxzr, dyxzr, dymzr, dxyzr, dxxzr, dxmzr, dmmzr, dmyzr, dmxzr
