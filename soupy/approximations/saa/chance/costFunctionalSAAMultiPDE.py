...
class CostFunctionalSAAMultiPDE(CostFunctional):
    """
    ...
    def __init__(self, parameter, Vh, pde, qoi, prior, penalization, tol=1e-9):
        ...
        self.pde = pde
        self.N_pde = len(pde)
        self.generate_optimization = pde[0].generate_optimization
        self.qoi = qoi
        self.prior = prior
        self.penalization = penalization
        self.z = pde[0].generate_optimization()
        self.tol = tol

        self.m = prior.mean
        self.x = pde[0].generate_state()
        self.y = pde[0].generate_state()
        self.x_all = [self.x, self.m, self.y, self.z]

        self.z_dir = pde[0].generate_optimization()
        self.z_diff = pde[0].generate_optimization()
        self.dz = None
        ...
        self.mean, self.var = [None]*self.N_pde, [None]*self.N_pde
        self.m, self.x, self.y, self.Q = [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde, [None]*self.N_pde
        for n_pde in range(self.N_pde):
            self.mean[n_pde] = np.zeros(self.N_mc)
            self.var[n_pde] = np.zeros(self.N_mc)

            self.m[n_pde] = []
            self.x[n_pde] = []
            self.y[n_pde] = []
            self.Q[n_pde] = np.zeros(self.N_mc)
            ...
            for i in range(self.N_mc):
                ...
                self.m[n_pde].append(sample)

                self.x[n_pde].append(pde[n_pde].generate_state())
                self.y[n_pde].append(pde[n_pde].generate_state())
    ...
    def objective(self):
        ...
        for n_pde in range(self.N_pde):
            for i in range(self.N_mc):
                x_all = [self.x[n_pde][i], self.m[n_pde][i], self.y[n_pde][i], self.z]
                ...
                self.mean[n_pde][i] = Q_i
                self.var[n_pde][i] = Q_i**2

            cost += np.mean(self.mean[n_pde]) \
                         + self.beta*np.mean(self.var[n_pde]) - self.beta*(np.mean(self.mean[n_pde]))**2
    ...
    def solve_ymc(self):
        for n_pde in range(self.N_pde):
            for i in range(self.N_mc):
                ...
                rhs[:] *= 1./self.N_mc*(1. + 2.*self.beta*self.mean[n_pde][i] - 2.*self.beta*np.mean(self.mean[n_pde]))
                self.pde[n_pde].solveAdj(self.y[n_pde][i], x_all, rhs, self.tol)
    ...
    def costGradient(self, z):
        ...
        for n_pde in range(self.N_pde):
            for i in range(self.N_mc):
                x_all = [self.x[n_pde][i], self.m[n_pde][i], self.y[n_pde][i], self.z]
                pdzq = self.pde[n_pde].generate_optimization()
                self.qoi[n_pde].grad_optimization(x_all,pdzq)
                dzq.axpy(1.0/self.N_mc + 2*self.beta/self.N_mc*self.qoi[n_pde].eval(x_all) - 2*self.beta*np.mean(self.Q), pdzq)
        ...
        for n_pde in range(self.N_pde):
            for i in range(self.N_mc):
                m_fun = vector2Function(self.m[n_pde][i], self.Vh[PARAMETER])
                x_fun = vector2Function(self.x[n_pde][i], self.Vh[STATE])
                y_fun = vector2Function(self.y[n_pde][i], self.Vh[ADJOINT])
    ...
    def costHessian(self, z, z_dir, FD=False):
        ...
        if FD:
            ...
            for n_pde in range(self.N_pde):
                ...
                m_fun = vector2Function(self.m[n_pde][0], self.Vh[PARAMETER])
                x_fun = vector2Function(self.x[n_pde][0], self.Vh[STATE])
                y_fun = vector2Function(self.y[n_pde][0], self.Vh[ADJOINT])
            ...
            #
            #     q_star_form = dl.derivative(self.qoi[n_pde].form(x_fun), x_fun, x_star)
            ...
            #
            #     qx_form = dl.derivative(self.qoi[n_pde].form(x_fun), x_fun, x_test)
            ...
            dzzp = self.generate_optimization()
    ...
    def init_vector(self, z, dim):
        ...
        # znew = self.generate_optimization()
        # z.init(znew.size())
