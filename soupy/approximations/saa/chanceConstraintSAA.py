...
class ChanceConstraintSAA(CostFunctional):
    ...
    def __init__(self, parameter, Vh, pde, qoi, qoi_constraint, prior, penalization, tol=1e-9):
        ...
        if self.c_gamma > 0:
            ...
            for i in range(self.N_cc):
                ...
                prior.sample(noise, sample, add_mean=True)
                self.m_constraint[i] = sample
                self.x_constraint.append(pde.generate_state())
                self.y_constraint.append(pde.generate_state())
    ...
    def constraint(self):
        ...
        for i in range(self.N_cc):
            x = self.pde.generate_state()
            x_all = [self.x_constraint[i], self.m_constraint[i], self.y_constraint[i], self.z]
            self.pde.solveFwd(x, x_all, self.tol)
            self.x_constraint[i] = x
            x_all[STATE] = x
            self.f[i] = self.qoi_constraint.eval(x_all)

        chance = -self.c_alpha

        for i in range(self.N_cc):
            l_beta = 1/(1+np.exp(-2*self.c_beta*self.f[i]))
        ...
        if self.mpi_rank == 0:
            print("P(f>=0) = ", np.double(self.f>=0).sum()/self.N_cc, " E[\ell_beta(f)] = ", chance + self.c_alpha)
    ...
    def solve_ymc(self):
        ...
        if self.c_gamma > 0:
            ...
            for i in range(self.N_cc):
                ...
                rhs.axpy(self.c_f[i], rhs_constraint)

                self.pde.solveAdj(self.y_constraint[i], x_all, rhs, self.tol)
    ...
    def costGradient(self, z):
        ...
        if self.c_gamma > 0:
            ...
            for j in range(self.N_cc):
                ...
                dl_beta = 2 * self.c_beta * np.exp(-2 * self.c_beta * self.f[j]) / \
                          (1 + np.exp(-2 * self.c_beta * self.f[j])) ** 2
                self.c_f[j] = 1. / self.N_cc * ds_gamma * dl_beta
        ...
        if self.c_gamma > 0:
            ...
            for i in range(self.N_cc):
                ...
                dzf.axpy(self.c_f[i], pdzf)

                m_fun = vector2Function(self.m_constraint[i], self.Vh[PARAMETER])
                x_fun = vector2Function(self.x_constraint[i], self.Vh[STATE])
                y_fun = vector2Function(self.y_constraint[i], self.Vh[ADJOINT])
                f_form = self.pde.residual(x_fun, m_fun, y_fun, z_fun)
