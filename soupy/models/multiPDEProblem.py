...
# import sys
# sys.path.append('../')
from ..utils.variables import STATE, PARAMETER
...
class MultiPDEProblem(PDEProblem):
    ...
    def solveFwd(self, state, x, tol):
        ...
        for k in range(self.n_problems):
            self.problems[k].solveFwd(state.data[k], [u.data[k], m, p.data[k] ], tol )
    ...
    def solveAdj(self, adj, x, adj_rhs, tol):
        ...
        u, m, p = x
        for k in range(self.n_problems):
            self.problems[k].solveAdj(adj.data[k], [u.data[k], m, p.data[k] ], adj_rhs.data[k], tol )
    ...
    def solveIncremental(self, out, rhs, is_adj, mytol):
        ...
        """
        for k in range(self.n_problems):
            self.problems[k].solveIncremental(out.data[k], rhs.data[k], is_adj, mytol)
